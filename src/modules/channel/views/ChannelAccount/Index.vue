...
import { deleteByIds, page, realDelete, recovery } from '~/channel/api/ChannelAccount.ts'
...
const IndexName = 'ChannelAccount:Index'
...
const options = ref<MaProTableOptions>({
  ...
  header: {
    mainTitle: () => t('channelAccount.index'),
  },
  // 表格参数
  tableOptions: {
    on: {
...
<template>
  <div class="mine-layout pt-3">
    <MaProTable ref="proTableRef" :options="options" :schema="schema">
      <template #actions>
        <el-button
          v-auth="['channel:channel_account:save']"
          type="primary"
          @click="() => {
      ...
      <template #toolbarLeft>
        <el-button-group>
          <el-button
            v-auth="['channel:channel_account:delete']"
            type="danger"
            plain
          ...
          <el-button
            v-if="isRecovery"
            v-auth="['channel:channel_account:recovery']"
            type="success"
            plain
            :disabled="selections.length < 1"
