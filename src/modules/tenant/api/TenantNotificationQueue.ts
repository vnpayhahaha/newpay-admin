...
export function page(params: TenantNotificationQueueVo): Promise<ResponseStruct<TenantNotificationQueueVo[]>> {
return useHttp().get('/admin/tenant/tenant_notification_queue/list', { params })
}

// tenant_notification_queue新增
export function create(data: TenantNotificationQueueVo): Promise<ResponseStruct<null>> {
  return useHttp().post('/admin/tenant/tenant_notification_queue', data)
}

// tenant_notification_queue编辑
export function save(id: number, data: TenantNotificationQueueVo): Promise<ResponseStruct<null>> {
    return useHttp().put(`/admin/tenant/tenant_notification_queue/${id}`, data)
}

// tenant_notification_queue删除
export function deleteByIds(ids: number[]): Promise<ResponseStruct<null>> {
      return useHttp().delete('/admin/tenant/tenant_notification_queue', { data: ids })
}