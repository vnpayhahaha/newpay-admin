...
import { deleteByIds, page } from '~/tenant/api/TenantNotificationQueue.ts'
...
defineOptions({ name: 'tenant:tenant_notification_queue' })
...
const options = ref<MaProTableOptions>({
  ...
  header: {
    mainTitle: () => t('tenant_notification_queue.index'),
  },
  // 表格参数
  tableOptions: {
    on: {
...
<template>
  <div class="mine-layout pt-3">
    <MaProTable ref="proTableRef" :options="options" :schema="schema">
      <template #actions>
        <el-button
          v-auth="['tenant:tenant_notification_queue:save']"
          type="primary"
          @click="() => {
      ...
      <template #toolbarLeft>
        <el-button
          v-auth="['tenant:tenant_notification_queue:delete']"
          type="danger"
          plain
