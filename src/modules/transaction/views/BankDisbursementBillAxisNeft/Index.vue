...
import { deleteByIds, page } from '~/transaction/api/BankDisbursementBillAxisNeft.ts'
...
defineOptions({ name: 'transaction:bank_disbursement_bill_axis_neft' })
...
const options = ref<MaProTableOptions>({
  ...
  header: {
    mainTitle: () => t('bank_disbursement_bill_axis_neft.index'),
  },
  // 表格参数
  tableOptions: {
    on: {
...
<template>
  <div class="mine-layout pt-3">
    <MaProTable ref="proTableRef" :options="options" :schema="schema">
      <template #actions>
        <el-button
          v-auth="['transaction:bank_disbursement_bill_axis_neft:save']"
          type="primary"
          @click="() => {
      ...
      <template #toolbarLeft>
        <el-button
          v-auth="['transaction:bank_disbursement_bill_axis_neft:delete']"
          type="danger"
          plain
