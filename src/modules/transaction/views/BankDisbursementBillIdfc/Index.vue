...
import { deleteByIds, page } from '~/transaction/api/BankDisbursementBillIdfc.ts'
...
defineOptions({ name: 'transaction:bank_disbursement_bill_idfc' })
...
const options = ref<MaProTableOptions>({
  ...
  header: {
    mainTitle: () => t('bank_disbursement_bill_idfc.index'),
  },
  // 表格参数
  tableOptions: {
    on: {
...
<template>
  <div class="mine-layout pt-3">
    <MaProTable ref="proTableRef" :options="options" :schema="schema">
      <template #actions>
        <el-button
          v-auth="['transaction:bank_disbursement_bill_idfc:save']"
          type="primary"
          @click="() => {
      ...
      <template #toolbarLeft>
        <el-button
          v-auth="['transaction:bank_disbursement_bill_idfc:delete']"
          type="danger"
          plain
