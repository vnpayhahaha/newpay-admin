...
import { deleteByIds, page } from '~/transaction/api/BankDisbursementBillIobOther.ts'
...
defineOptions({ name: 'transaction:bank_disbursement_bill_iob_other' })
...
const options = ref<MaProTableOptions>({
  ...
  header: {
    mainTitle: () => t('bank_disbursement_bill_iob_other.index'),
  },
  // 表格参数
  tableOptions: {
    on: {
...
<template>
  <div class="mine-layout pt-3">
    <MaProTable ref="proTableRef" :options="options" :schema="schema">
      <template #actions>
        <el-button
          v-auth="['transaction:bank_disbursement_bill_iob_other:save']"
          type="primary"
          @click="() => {
      ...
      <template #toolbarLeft>
        <el-button
          v-auth="['transaction:bank_disbursement_bill_iob_other:delete']"
          type="danger"
          plain
