...
import { deleteByIds, page } from '~/transaction/api/BankDisbursementBillYesmsme.ts'
...
defineOptions({ name: 'transaction:bank_disbursement_bill_yesmsme' })
...
const options = ref<MaProTableOptions>({
  ...
  header: {
    mainTitle: () => t('bank_disbursement_bill_yesmsme.index'),
  },
  // 表格参数
  tableOptions: {
    on: {
...
<template>
  <div class="mine-layout pt-3">
    <MaProTable ref="proTableRef" :options="options" :schema="schema">
      <template #actions>
        <el-button
          v-auth="['transaction:bank_disbursement_bill_yesmsme:save']"
          type="primary"
          @click="() => {
      ...
      <template #toolbarLeft>
        <el-button
          v-auth="['transaction:bank_disbursement_bill_yesmsme:delete']"
          type="danger"
          plain
