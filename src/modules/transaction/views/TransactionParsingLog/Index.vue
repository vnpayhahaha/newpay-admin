...
import { deleteByIds, page } from '~/transaction/api/TransactionParsingLog.ts'
...
defineOptions({ name: 'transaction:transaction_parsing_log' })
...
const options = ref<MaProTableOptions>({
  ...
  header: {
    mainTitle: () => t('transaction_parsing_log.index'),
  },
  // 表格参数
  tableOptions: {
    on: {
...
<template>
  <div class="mine-layout pt-3">
    <MaProTable ref="proTableRef" :options="options" :schema="schema">
      <template #actions>
        <el-button
          v-auth="['transaction:transaction_parsing_log:save']"
          type="primary"
          @click="() => {
      ...
      <template #toolbarLeft>
        <el-button
          v-auth="['transaction:transaction_parsing_log:delete']"
          type="danger"
          plain
