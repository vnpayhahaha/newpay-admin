...
import { deleteByIds, page } from '~/transaction/api/TransactionParsingRules.ts'
...
defineOptions({ name: 'transaction:transaction_parsing_rules' })
...
const options = ref<MaProTableOptions>({
  ...
  header: {
    mainTitle: () => t('transaction_parsing_rules.index'),
  },
  // 表格参数
  tableOptions: {
    on: {
...
<template>
  <div class="mine-layout pt-3">
    <MaProTable ref="proTableRef" :options="options" :schema="schema">
      <template #actions>
        <el-button
          v-auth="['transaction:transaction_parsing_rules:save']"
          type="primary"
          @click="() => {
