...
import { deleteByIds, page } from '~/transaction/api/TransactionVoucher.ts'
...
defineOptions({ name: 'transaction:transaction_voucher' })
...
const options = ref<MaProTableOptions>({
  ...
  header: {
    mainTitle: () => t('transaction_voucher.index'),
  },
  // 表格参数
  tableOptions: {
    on: {
...
<template>
  <div class="mine-layout pt-3">
    <MaProTable ref="proTableRef" :options="options" :schema="schema">
      <template #actions>
        <el-button
          v-auth="['transaction:transaction_voucher:save']"
          type="primary"
          @click="() => {
      ...
      <template #toolbarLeft>
        <el-button
          v-auth="['transaction:transaction_voucher:delete']"
          type="danger"
          plain
