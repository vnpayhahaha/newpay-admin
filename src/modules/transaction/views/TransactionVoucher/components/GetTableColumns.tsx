...
export default function getTableColumns(dialog: UseDialogExpose, formRef: any, t: any): MaProTableColumns[] {
  ...
  return [
      ...
      cellRender: ({ row }) => {
        return (
          <div class="text-align-left" style={{ display: 'flex', alignItems: 'center' }}>
            <div class="ml-5" style={{ flex: 1, minWidth: 0 }}>
              <p>
                <el-text class="mx-1" type="primary">{row.channel_account?.merchant_id || ''}</el-text>
              </p>
            </div>
          </div>
        )
      ...
      cellRender: ({ row }) => {
        return (
          <div class="text-align-left" style={{ display: 'flex', alignItems: 'center' }}>
            <div class="ml-5" style={{ flex: 1, minWidth: 0 }}>
              <p>
                <el-text class="mx-1" type="primary">{row.bank_account?.account_holder || ''}</el-text>
              </p>
              <p>
                <el-text class="mx-1" truncated>{row.bank_account?.account_number}</el-text>
              </p>
              <p>
                <el-text class="mx-1" truncated>{row.bank_account?.bank_code}</el-text>
              </p>
              <p>
                <el-text class="mx-1" truncated>{row.bank_account?.upi_id}</el-text>
              </p>
            </div>
          </div>
        )
      ...
      cellRender: ({ row }) => {
        return (
          <div class="text-align-left" style={{ display: 'flex', alignItems: 'center' }}>
            <div class="ml-5" style={{ flex: 1, minWidth: 0 }}>
              <p>
                <el-text class="mx-1" type="primary">{row.collection_card_no}</el-text>
              </p>
              <p>
                <el-text class="mx-1" truncated>{row.collection_amount}</el-text>
              </p>
              <p>
                <el-text class="mx-1" truncated>{row.collection_fee}</el-text>
              </p>
              <p>
                <el-text class="mx-1" truncated>{row.collection_time}</el-text>
              </p>
            </div>
          </div>
        )
    ...
    { label: () => t('transaction_voucher.order_no'), prop: 'order_no', width: 220 },
    { label: () => t('transaction_voucher.content'), prop: 'content' },
    { label: () => t('transaction_voucher.operation_admin_id'), prop: 'operation_admin_id' },
    { label: () => t('transaction_voucher.created_at'), prop: 'created_at', width: 180 },
    {
