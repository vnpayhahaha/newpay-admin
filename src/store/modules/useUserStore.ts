...
const useUserStore = defineStore("useUserStore", () => {
  ...
  function getDropdownMenuState(key: string) {
    return dropdownMenuState.value[key] !== undefined
      ? dropdownMenuState.value[key]
      : undefined;
  }
    ...
    return new Promise((resolve, reject) => {
      loginApi(data)
        .then(async (res) => {
          ...
          cache.set("expire", useDayjs().unix() + res.data.expire_at, {
            exp: res.data.expire_at,
          });
  ...
  async function requestUserInfo(): Promise<void> {
    try {
      ...
      setUserInfo(data);
      if (
        (setting.getSettings("app")?.loadUserSetting ?? true) &&
        data.backend_setting
      ) {
        setUserSetting(
          data?.backend_setting.length === 0 ? null : data.backend_setting
        );
      }
      await refreshMenu();
      await refreshRole();
  ...
  function saveSettingToSever(lang?: string) {
    ...
    useHttp()
      .post("/admin/permission/update", { backend_setting })
      .then(() => {
